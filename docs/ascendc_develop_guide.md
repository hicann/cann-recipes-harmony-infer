...
直接参考 ops/ascendc/src/slice_gelu_custom工程
...
│  └─onnx_plugin
│          │──CMakeLists.txt
│          │──slice_gelu_mul_plugin.cc
│
├─op_host
│      │──CMakeLists.txt
│      │──slice_gelu_mul.cpp
│      │──slice_gelu_mul_tiling.h
│
├─op_kernel
│      │──CMakeLists.txt
│      │──slice_gelu_mul.cpp
...
```
> **说明**：上述目录结构中的粗体文件op_host/slice_gelu_mul_tiling.h、op_host/slice_gelu_mul.cpp、op_kernel/slice_gelu_mul.cpp、framework/onnx_plugin/slice_gelu_mul_plugin.cc为后续算子开发过程中需要修改的文件，其他文件无需修改。也可以参考本仓中其他样例算子目录。
