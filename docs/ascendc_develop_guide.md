...
参考本仓中其他样例算子目录 
ops/ascendc/src/slice_gelu_custom
    ```text
    AddCustom 
...
| Tiling | （Kirin当前版本暂不支持）描述Kirin AI处理器上算子的输入/输出数据切分、分块计算、多核并行等策略，主要是为了满足片上存储限制和计算pipeline的需求，最大化计算并行性和数据局部性（data locality 或 data reuse），从而发挥硬件的极致性能。 |
...
- **方式：采用脚本生成input/golden数据。**
    ...
    可以参考ops/AddKernelInvocation/scripts/gen_data.py
