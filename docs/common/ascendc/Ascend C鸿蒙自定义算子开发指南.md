...
进行自定义算子开发前，需要完成驱动及CANN软件包的安装，请参考[环境准备](https://gitcode.com/cann/cann-recipes-harmony-infer/blob/master/docs/common/%E7%8E%AF%E5%A2%83%E5%87%86%E5%A4%87.md)完成环境准备。
...
2. 使用msOpGen工具生成AddCustom算子的开发工程。
    ...
    样例：
    ```
    msopgen gen -i ./add_custom.json -c ai_core-KirinX90 -out ./AddCustom    
    ```
...
该算子json配置模板默认采用**固定输入/输出顺序**格式，具体格式和参数的介绍请参见[固定输入/输出顺序的算子json配置](#固定输入输出顺序的算子json配置)。
...
- **场景3：支持原地算子格式的输入。**
  ...
  > 暂不支持该方式设置输入。
    ...
    当算子为[原地算子(in-place op)](#算子调试概述)时，输入和输出地址一样，配置算子json时“outputs”和“inputs”除了“data_file”不同，其他配置项均保持一致。配置示例如下。
...
- **场景3：支持原地算子格式的输入。**
  当算子为[原地算子(in-place op)](#算子调试概述)时，输入和输出地址一样，配置算子json时除了“param_type”和“data_file”不同，其他配置项均保持一致。配置示例如下。
