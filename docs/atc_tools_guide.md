...
##  环境准备
1. 参见[环境准备](./quick_install.md)进行环境搭建，并确保开发套件Ascend-cann-toolkit安装完成。该场景下ATC工具安装在`${install_path}/latest/bin`目录下；其中，`${install_path}`请替换为CANN软件安装目录，以root安装举例，安装目录为：`/usr/local/Ascend/ascend-toolkit`。
