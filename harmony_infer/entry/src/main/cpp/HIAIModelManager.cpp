...
namespace {
    ...
    OH_NN_ReturnCode SetInputTensorData(std::vector<NN_Tensor *> inputTensors,
                                        std::vector<std::pair<uint8_t *, size_t>> inputData) {
        for (size_t i = 0; i < inputTensors.size(); ++i) {
            ...
            OH_NNTensor_GetSize(inputTensors[i], &dataSize);
               if (data == nullptr || dataSize != inputData[i].second) {
...
OH_NN_ReturnCode HIAIModelManager::InitIOTensors(std::vector<std::pair<uint8_t *, size_t>> inputData)
{
...
OH_NN_ReturnCode HIAIModelManager::GetResult(uint8_t* outputDat)
{
    if (outputDat == nullptr) {
        OH_LOG_ERROR(LOG_APP, "GetResult failed: outputDat is nullptr.");
        return OH_NN_INVALID_PARAMETER;
    }
    size_t offset = 0;      // keep track of total copied data
    bool hasFailure = false;
    for (size_t i = 0; i < outputTensors_.size(); ++i) {
        void *tensorData = OH_NNTensor_GetDataBuffer(outputTensors_[i]);
        if (tensorData == nullptr) {
            OH_LOG_ERROR(LOG_APP, "Tensor[%zu]: GetDataBuffer failed, skip this tensor.", i);
            hasFailure = true;
            continue;
        }
        size_t tensorSize = 0;
        OH_NN_ReturnCode ret = OH_NNTensor_GetSize(outputTensors_[i], &tensorSize);
        if (ret != OH_NN_SUCCESS || tensorSize == 0) {
            OH_LOG_ERROR(LOG_APP, "Tensor[%zu]: GetSize failed (ret=%d, size=%zu), skip this tensor.", i, ret, tensorSize);
            hasFailure = true;
            continue;
        }

        OH_LOG_INFO(LOG_APP, "Tensor[%zu]: copying size=%zu to outputDat offset=%zu", i, tensorSize, offset);
        memcpy(outputDat + offset, static_cast<uint8_t*>(tensorData), tensorSize);
        
        OH_LOG_WARN(LOG_APP, "GetResult partially completed. this is %{public}d.", outputDat);
        offset += tensorSize;
    }
    if (hasFailure) {
        OH_LOG_WARN(LOG_APP, "GetResult partially completed. Total bytes copied=%zu. Some tensors failed to copy.", offset);
        // 根据你自己的错误码规范，可替换为半成功的返回码
        return OH_NN_SUCCESS; 
    } else {
        OH_LOG_INFO(LOG_APP, "GetResult success. All tensors copied. Total bytes=%zu", offset);
        return OH_NN_SUCCESS;
    }
}
