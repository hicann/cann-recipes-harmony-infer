...
ATTR_DEFAULT = {
    'bool': 'False', 'int': '0', 'float': '0.0', 'list_int': '[]',
...
class AdpBuilder(opdesc_parser.OpDesc):
    ...
    def _trans_list_bool(self, para_value: str) -> str:
        inner_list = para_value[1:-1].split(",")
        translated = []
        for value in inner_list:
            translated.append(value.capitalize())
        ret = ",".join(translated)
        return f"[{ret}]"

    def _trans_list_list_bool(self, para_value: str) -> str:
        inner_list = para_value[1:-1].split("[")
        translated = []
        for list_bool in inner_list:
            translated.append(self._trans_list_bool(f"[{list_bool}"))
        ret = ','.join(translated)
        return f"[{ret}]"
    ...
    def _attr_argpack(self: any, default: bool = True) -> list:
        ...
        for i in range(len(self.attr_list)):
            ...
            arg = att
            if default and self.argsdefv[i + argidx] is not None:
                arg_type=self.attr_val.get(att).get('type')
                if arg_type == 'str':
                    arg = f'{arg}="{self.argsdefv[i + argidx]}"'
                elif arg_type == 'bool':
                    arg = f'{arg}={self.argsdefv[i + argidx].capitalize()}'
                elif arg_type == "list_bool":
                    translated = self._trans_list_bool(self.argsdefv[i + argidx])
                    arg = f'{arg}={translated}'
                elif arg_type == "list_list_bool":
                    translated = self._trans_list_list_bool(self.argsdefv[i + argidx])
                    arg = f'{arg}={translated}'
                else:
                    arg = f'{arg}={self.argsdefv[i + argidx]}'
            args.append(arg)
